...
def get_large_paths_info(directory_path, limit_size, dirs_consider=True):
    large_files = []
    large_dirs = []
    for dirname, subdirs, filenames in os.walk(directory_path, followlinks=True):
        for filename in filenames:
            full_filename = os.path.join(dirname, filename)
            if (size := os.path.getsize(full_filename)) > limit_size and not os.path.islink(full_filename):
                large_files.append((size, full_filename))

        if dirs_consider:
            if (size := get_directory_full_size(dirname)) > limit_size and not os.path.islink(dirname):
